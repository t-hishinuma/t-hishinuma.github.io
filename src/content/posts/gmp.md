...
仮数部については固定で，指数部を指定していくことになります．
そのため例題などではよく円周率などが扱われる印象があります．

ここではC++版の`gmpgxx.h`を使ってGMPの多倍長浮動小数点型である`mpf_t`のC++ Wrapper, `mpf_class`を使ってみます．
...
`_mp_prec`は指定した精度です．ただし`mpf_set_default_prec()`には仮数部のbit数を入力するのに対し，
`_mp_prec`には`_mp_d`の配列の長さ+1が入ります．\
つまり指数部64bitなら1+1=2, 128bitなら2+1=3, 512bitなら8+1=9です．

`_mp_size`は`_mp_d`の配列にどこまで値が入っているかです．
...
ただし，このあたりの挙動をある程度把握しないと，演算時間がスケールしなかったり，入力によって性能が安定しないかのような印象を受けるでしょう．
